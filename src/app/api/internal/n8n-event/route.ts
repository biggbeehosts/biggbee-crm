...
import { z } from "zod";
import { ANALYTICS_EVENT_TYPES, DEFAULT_WORKSPACE_ID } from "@/types";
...
const BodySchema = z.object({
  ...
  /** Phase A: optional until the n8n workflow is updated to send it explicitly (carried from the
   *  campaign row it already reads) -- defaults to the default workspace so this endpoint keeps
   *  working unchanged for the current single-workspace production state. Once n8n sends it, this
   *  becomes the real scoping key for the lead-suppression writes below. */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (email) {
    if (type === "sent" && n8nExecutionId) {
      updateLeadFields(workspaceId, email, { n8nExecutionId }).catch(() => {});
    } else if (type === "hard_bounce") {
      updateLeadFields(workspaceId, email, { bounceType: "Hard", bouncedAt: at, suppressedReason: "Hard bounce" }).catch(() => {});
      logAudit({ actor: "system", action: "lead.suppressed", target: email, success: true, details: { workspaceId, reason: "Hard bounce", campaignId, n8nExecutionId } }).catch(() => {});
    } else if (type === "soft_bounce") {
      updateLeadFields(workspaceId, email, { bounceType: "Soft" }).catch(() => {});
      ...
      updateLeadFields(workspaceId, email, { bounceType: "Deferred", bouncedAt: at }).catch(() => {});
    } else if (type === "complaint") {
      updateLeadFields(workspaceId, email, { complaintAt: at, suppressedReason: "Complaint" }).catch(() => {});
      logAudit({ actor: "system", action: "lead.suppressed", target: email, success: true, details: { workspaceId, reason: "Complaint", campaignId, n8nExecutionId } }).catch(() => {});
    } else if (type === "unsubscribe") {
      updateLeadFields(workspaceId, email, { suppressedReason: "Unsubscribed" }).catch(() => {});
      logAudit({ actor: "system", action: "lead.suppressed", target: email, success: true, details: { workspaceId, reason: "Unsubscribed", campaignId, n8nExecutionId } }).catch(() => {});
    } else if (type === "demo_sent") {
      updateLeadFields(workspaceId, email, { demoSent: true, demoSentAt: at }).catch(() => {});
    }
  }
