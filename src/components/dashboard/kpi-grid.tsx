...
export function KpiGrid({ metrics }: { metrics: DashboardMetrics }) {
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
      <StatCard label="Total Leads" value={formatNumber(metrics.totalLeads)} icon={Users} tone="accent" />
      <StatCard label="Emails Sent" value={formatNumber(metrics.emailsSent)} icon={Send} tone="success" />
      <StatCard label="Interested" value={formatNumber(metrics.interested)} icon={ThumbsUp} />
